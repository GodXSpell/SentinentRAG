...
def decide(score: float, retry_count: int) -> models.CorrectionDecision:
    """
    ...
    """
    if score >= models.DIRECT_THRESHOLD:
        return models.CorrectionDecision(
            state=models.CorrectionState.DIRECT,
            reason=f"Score {score:.2f} >= DIRECT_THRESHOLD {models.DIRECT_THRESHOLD:.2f}, proceeding directly.",
        )

    if score < models.RETRY_FLOOR:
        return models.CorrectionDecision(
            state=models.CorrectionState.REFUSED,
            reason=f"Score {score:.2f} < RETRY_FLOOR {models.RETRY_FLOOR:.2f}, refusing outright.",
        )

    # Middle band: RETRY_FLOOR <= score < DIRECT_THRESHOLD
    if retry_count < models.MAX_RETRIES:
        return models.CorrectionDecision(
            state=models.CorrectionState.RETRY,
            reason=(
                f"Score {score:.2f} in middle band, "
                f"retry_count {retry_count} < MAX_RETRIES {models.MAX_RETRIES}, retrying."
            ),
        )
    ...
    return models.CorrectionDecision(
        ...
        reason=(
            ...
            f">= MAX_RETRIES {models.MAX_RETRIES}, refusing after retry."
        ),
    )